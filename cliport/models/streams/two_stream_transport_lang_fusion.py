...
class cjjtransporter(TwoStreamTransportLangFusion):
    """Two Stream Transport (a.k.a Place) module with lateral connections"""
...
class cjj_two_transporter(TwoStreamTransportLangFusion):
    """Two Stream Transport (a.k.a Place) module with lateral connections"""
    ...
    def transport(self, in_tensor, crop, l):
        ...
        key_out_two = self.blip2(batch)
        key_out_two = torch.nn.functional.pad(key_out_two, (self.pad_size, self.pad_size, self.pad_size, self.pad_size), mode='reflect')
        key_out_two = self.conv(key_out_two)
        logits = self.fusion_key(key_out_one, key_out_two)
